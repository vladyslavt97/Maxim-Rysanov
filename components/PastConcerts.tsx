...
export default function PastConcerts({}: Props) {
  ...
  return (
    <div className='rounded--lg absolute right-0'>
      <button className='py-2 pl-3 pr-1 bg-slate-500 rounded-lg flex flex-row text-center justify-center items-center text-white text-sm' onClick={e=>setOpen(!open)}>Past concerts<MdKeyboardArrowDown className='ml-1 text-white'/></button>
      {open && <div className='flex flex-col bg-white text-black rounded-lg'>
        ...
        <Link href="/concerts-2021" className='hover:bg-slate-300 py-2 hover:rounded-lg border-b-2 border-gray-300 text-sm text-center rounded-b-lg'><button onClick={e=>setOpen(!open)}>Concerts 2021</button></Link>
